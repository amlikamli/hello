...
class Home extends CI_Controller
{
	...
	function deals($city_name="")
	{
		...
		$this->db->order_by("deal_start_date", "desc");
	...
	{	
		...
		if(count($deal_detail)!=0)
		{
			...
			if(isset($data["selected_city"]) && $data["selected_city"]!="")
			{
				...
				$side_deal_query=$this->common_model->getTableData("deals",$other_contision,NULL,NULL,array(13));
		...
		if(count($mergent_data))
		{
			...
			$data["mergent_map"]=$mergent_data[0]->view_map;
			
			$data["mergent_lat"]=$mergent_data[0]->division_lat;
			
			$data["mergent_lng"]=$mergent_data[0]->division_lng;
		}
	...
	{
		if($this->input->post("form_name")=="merchant_login")
		{
			extract($this->input->post());
			
			$conditions 		=  array('email'=>trim($email),'password' => md5(trim($password)),'status' => '1');
				
			$query				= $this->user_model->getMerchants($conditions);
			...
			if(count($row)!=0)
			{
				if($row->email==$email && $row->password==md5($password))
				{
					...
					$values = array('merchant_id'=>$row->id,'logged_in'=>TRUE);
	...
	{
		if($this->input->post("form_name")=="agent_login")
		{
			extract($this->input->post());
			
			$conditions 		=  array('email'=>trim($email),'password' => md5(trim($password)),'status' => '1');
				
			$query				= $this->user_model->getAgents($conditions);
			...
			if(count($row)!=0)
			{
				if($row->email==$email && $row->password==md5($password))
				{
					...
					$values = array('agent_id'=>$row->id,'logged_in'=>TRUE);
					$this->session->set_userdata($values);
	...
	function merchant_signup()
	{
		...
		if($this->input->post("sign_upmerchant"))
		{
			
			$this->form_validation->set_rules("company",'Company name is required','required');
			...
			$this->form_validation->set_rules("telephone",'Contact number is required','required');
			
			$this->form_validation->set_rules("address",'Address is required','required');
			...
			if($this->form_validation->run())
			{
				...
				$merchant_details=$this->common_model->getTableData("merchants",$conditions)->result();
				
				if(count($merchant_details)!=0)
				{
					$this->session->set_flashdata('flash_message', $this->common_model->admin_flash_message('error','Please try other email id.It already Exists'));
					
					redirect("merchant_signup");	
				}
				else
				{
					//$user_name=$this->common_model->create_unique_slug($first_name,"users","user_name");
					...
					$insertData=array("first_name"=>$first_name,"last_name"=>$last_name,"email"=>$email, "password"=>md5($password), "phone_no"=>$telephone,"address"=>$address,"company_name"=>$company,"site_url"=>$website,"company_activities"=>$company_activities,"mobile_num"=>$cellphone,"zipcode"=>$zipcode,"country"=>$country,"city"=>$city,"confirm_buying"=>$confirm_buying,"reasons"=>$reasons );
					
					
					if($_FILES["merchant_file"]["name"]!="")
					{
					
						if (!$this->upload->do_upload("merchant_file"))
						{
							$error = array('error' => $this->upload->display_errors());
							$this->session->set_flashdata('flash_message', $this->common_model->admin_flash_message('error',$error["error"]));
							redirect(admin_url("home/agent_signup"));
						}
						else
						{
						...
						if(count($upload_data!=0))
							$insertData['merchant_file']=$upload_data["file_name"];
				...
				{	
					...
					$this->session->set_flashdata('flash_message', $this->common_model->admin_flash_message('error','Error : Merchant Not Added'));
		...
		//$this->load->view("sign_up",$data);
		$this->load->view("merchant_signup");
	}
	...
	function agent_signup()
	{
		...
		if($this->input->post("sign_upagent"))
		{
			$this->form_validation->set_rules("first_name",'First name is required','required');
			
			$this->form_validation->set_rules("middle_name",'Middle name is required','required');
			...
			$this->form_validation->set_rules("telephone",'Telephone is required','required');
			...
			if($this->form_validation->run())
			{
				...
				$agent_details=$this->common_model->getTableData("agents",$conditions)->result();
				
				if(count($agent_details)!=0)
				{
					$this->session->set_flashdata('flash_message', $this->common_model->admin_flash_message('error','Please try other email id.It already Exsists'));
					
					redirect("agent_signup");	
				}
				else
				{
					//$user_name=$this->common_model->create_unique_slug($first_name,"users","user_name");
					...
					$insertData=array("first_name"=>$first_name,"middle_name"=>$middle_name,"last_name"=>$last_name,"email"=>$email,"telephone"=>$telephone,"address"=>$address,"date_birth"=>$date_birth,"gender"=>$gender,"cellphone"=>$cellphone,"education"=>$education,"familiar_groupbuying"=>$familiar_groupbuying,"worked_before"=>$worked_before,"previous_work"=>$previous_work);
					...
					if($_FILES["agent_file"]["name"]!="")
					{
						...
						if(count($upload_data!=0))
							$insertData['agent_file']=$upload_data["file_name"];
					...
					$result=$this->common_model->insertData('agents',$insertData);
				...
				{	
					...
					$this->session->set_flashdata('flash_message', $this->common_model->admin_flash_message('error','Error : Agent Not Added'));
	...
	function forget_pwd_agent()
	{
		if($this->input->post("forget_pwd_up")=="yes")
		{
			...
			if($this->form_validation->run())
			{
				...
				$agents_query=$this->common_model->getTableData("agents",$conditions);
		...
		$data["title"]="Forget agent password";
		
		$this->load->view("forget_password_agent",$data);
	}
	
	
	function forget_pwd_merchant()
	{
		if($this->input->post("forget_pwd_up")=="yes")
		{
			$this->form_validation->set_rules("forget_pwd_email",'Please Enter Valid Email id','required|valid_email');
			...
			if($this->form_validation->run())
			{
				...
				$merchants_query=$this->common_model->getTableData("merchants",$conditions);
				
				$merchants=$merchants_query->result();
				
				if(count($merchants)==0)
				{
					$this->session->set_flashdata('flash_message', $this->common_model->flash_message('error',"This email address doesn't exist in our database"));
					
					redirect('forget_pwd_merchant');
				}
				else
				{
					$rand_no=rand(1,111111);
		...
		$data["title"]="Forget merchant password";
		
		$this->load->view("forget_password_merchant",$data);
	}
	...
	function purchase($slug)
	{
		if(!isLoggedIn())
		{
			$this->session->set_flashdata('flash_message', $this->common_model->admin_flash_message('error','You Must Login to Purchase the deals'));	
	...
	function faq()
	{
		...
		$faq_query=$this->common_model->getTableDataFAQ("faq",$conditions);
